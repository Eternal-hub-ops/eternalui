...
local eternallib = {
	Themes = {
		...
		Dark = {
			...
			["Color Theme"] = Color3.fromRGB(168, 85, 247),
...
local Settings = eternallib.Settings
local Flags = eternallib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(eternallib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then eternallib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then eternallib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then eternallib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "Eternal Hub.json")
end
...
local Connections, Connection = {}, eternallib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(eternallib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = eternallib.Themes[eternallib.Save.Theme]

local function AddEle(Name, Func)
	eternallib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = eternallib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function eternallib:GetIcon(index)
...
function eternallib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	eternallib.Save.Theme = NewTheme
	SaveJson("Eternal Hub.json", eternallib.Save)
	Theme = eternallib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(eternallib.Instances, function(_,Val)
...
function eternallib:SetScale(NewScale)
...
function eternallib:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(eternallib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, eternallib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			eternallib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("Eternal Hub.json", eternallib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		eternallib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("Eternal Hub.json", eternallib.Save)
	end)
	...
	function Window:AddMinimizeButton(Configs)
		local Button = MakeDrag(Create("TextButton", ScreenGui, {
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			eternallib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(eternallib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		TIcon = eternallib:GetIcon(TIcon)
		...
		local function Tabs()
			...
			table.foreach(eternallib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(eternallib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(eternallib.Options, {type = "Section", Name = SectionName, func = Section})
